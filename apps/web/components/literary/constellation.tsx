...
export function Constellation() {
  const stars = useMemo(() => {
    ...
    const palette: Array<"amber" | "teal" | "rose"> = ["amber", "teal", "rose"];
    for (let i = 0; i < 220; i++) {
      arr.push({
    ...
    <div className="stage fade-in" style={{ overflow: "hidden" }}>
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse at center, rgba(20,15,30,0.4) 0%, var(--room) 80%)",
        }}
      />
      {stars.map((s, i) => (
        <div
          ...
          style={{
            left: `${s.left}%`,
            top: `${s.top}%`,
            animation: `lit-twinkle ${3 + s.blink}s ease-in-out infinite`,
      ...
      >
        <div>
          <span style={{ color: "var(--amber)" }}>◉</span> Sam · patterns
        </div>
        <div>
          <span style={{ color: "var(--teal)" }}>◉</span> Ellis · craft
        </div>
        <div>
          <span style={{ color: "var(--rose)" }}>◉</span> Kit · tensions
        </div>
      </div>
    </div>
  );
