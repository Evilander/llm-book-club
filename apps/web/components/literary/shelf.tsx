...
export function Shelf({ onPick }: ShelfProps) {
      ...
      >
        {SHELF_BOOKS.map((b, i) => {
          ...
          const style: CSSProperties = {
            ...
            position: "relative",
            transition:
              "transform 500ms var(--spring-paper), height 500ms var(--spring-paper), box-shadow 300ms ease",
            transform: `translateY(${pulled ? -14 : 0}px) translateY(${isHover ? -8 : 0}px) rotate(${tilt}deg)`,
      ...
      {hover !== null && (
        <div
          style={{
            ...
            fontStyle: "italic",
            fontSize: 14,
            animation: "lit-fade-in 300ms ease",
          }}
        >
          <div
            style={{
          ...
          <div style={{ color: "rgba(245,235,210,0.5)", fontSize: 12 }}>
            ...
            {SHELF_BOOKS[hover].audio > 0.5 && (
              <span style={{ color: "var(--teal)", marginLeft: 10 }}>
                ◉ audiobook · {Math.round(SHELF_BOOKS[hover].audio * 100)}%
                match
              </span>
            )}
            {SHELF_BOOKS[hover].pulled && (
