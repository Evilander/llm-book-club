...
class ReadingPrefs(Base):
    __tablename__ = "reading_prefs"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    user_id: Mapped[str] = mapped_column(String(64), index=True, unique=True)
    theme: Mapped[str] = mapped_column(String(64), default="cream-daylight")
    font_family: Mapped[str] = mapped_column(String(64), default="serif")
    font_size_px: Mapped[int] = mapped_column(Integer, default=20)
    line_height: Mapped[float] = mapped_column(Float, default=1.75)
    measure_ch: Mapped[int] = mapped_column(Integer, default=62)
    focus_reading: Mapped[bool] = mapped_column(Boolean, default=False)
    focus_reading_intensity: Mapped[int] = mapped_column(Integer, default=40)
...
class Book(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    # title / author / filename are Text (unbounded) — real ebook filenames in
    # the wild routinely exceed 500 chars (whole logline packed into the name).
