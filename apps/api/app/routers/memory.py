...
from ..services.reading_progress import choose_unit_for_section
...
class MarginaliaNoteResponse(BaseModel):
    agent: str
    name: str
    note: str | None = None


class BookMarginaliaResponse(BaseModel):
    book_id: str
    section_id: str | None = None
    marginalia: list[MarginaliaNoteResponse]
...
MARGINALIA_ROLES: tuple[tuple[MessageRole, str, str], ...] = (
    (MessageRole.FACILITATOR, "sam", "Sam"),
    (MessageRole.CLOSE_READER, "ellis", "Ellis"),
    (MessageRole.SKEPTIC, "kit", "Kit"),
)
SENTENCE_RE = re.compile(r"[^.!?]+(?:[.!?]+|$)")
CITATION_MARKER_RE = re.compile(
    r"(\[[^\]]+\]|\bcite\s*:|\bchunk(?:_id)?\s*[:#]\s*[\w-]+|\bchunk\s+(?:ref(?:erence)?\s*)?\d+)",
    re.IGNORECASE,
)
MAX_MARGINALIA_CHARS = 140
...
def _latest_discussion_session(db: Session, book_id: str) -> DiscussionSession | None:
    active = (
        db.query(DiscussionSession)
        .filter(DiscussionSession.book_id == book_id, DiscussionSession.is_active.is_(True))
        .order_by(DiscussionSession.updated_at.desc(), DiscussionSession.created_at.desc())
        .first()
    )
    if active:
        return active
    return (
        db.query(DiscussionSession)
        .filter(DiscussionSession.book_id == book_id)
        .order_by(DiscussionSession.updated_at.desc(), DiscussionSession.created_at.desc())
        .first()
    )


def _active_section_id(session: DiscussionSession | None) -> str | None:
    if not session or not session.section_ids:
        return None
    return session.section_ids[-1]


def _clean_note(sentence: str) -> str:
    note = re.sub(r"\s+", " ", sentence).strip().strip('"\u201c\u201d')
    note = note.rstrip(".").strip()
    if len(note) <= MAX_MARGINALIA_CHARS:
        return note
    return note[:MAX_MARGINALIA_CHARS].rstrip(" ,;:-.")
...
def _extract_marginalia_note(content: str) -> str | None:
    sentences = _split_sentences(content)
    if not sentences:
        return None
    clean = [
        _clean_note(sentence)
        for sentence in sentences
        if not CITATION_MARKER_RE.search(sentence)
    ]
    clean = [sentence for sentence in clean if sentence]
    if clean:
        return min(clean, key=len)
    fallback = _clean_note(sentences[0])
    return fallback or None


def _latest_role_messages(db: Session, session_id: str) -> dict[MessageRole, Message]:
    role_values = [role for role, _, _ in MARGINALIA_ROLES]
    messages = (
        db.query(Message)
        .filter(Message.session_id == session_id, Message.role.in_(role_values))
        .order_by(Message.created_at.desc())
        .all()
    )
    latest: dict[MessageRole, Message] = {}
...
@router.get("/books/{book_id}/marginalia", response_model=BookMarginaliaResponse)
async def get_book_marginalia(book_id: str, db: Session = Depends(get_db)):
    book = db.query(Book).filter(Book.id == book_id).first()
    if not book:
        raise HTTPException(status_code=404, detail="Book not found")

    session = _latest_discussion_session(db, book_id)
    messages = _latest_role_messages(db, session.id) if session else {}
    return BookMarginaliaResponse(
        book_id=book_id,
        section_id=_active_section_id(session),
        marginalia=[
            MarginaliaNoteResponse(
                agent=agent,
                name=name,
                note=_extract_marginalia_note(messages[role].content) if role in messages else None,
            )
            for role, agent, name in MARGINALIA_ROLES
        ],
    )
...
@router.post("/books/{book_id}/sections/{section_id}/progress")
async def update_section_progress(
    ...
    section = (
        db.query(Section)
        .filter(Section.book_id == book_id, Section.id == section_id)
        .first()
    )
    if not section:
    ...
    unit = choose_unit_for_section(units, section)
    if not unit:
        raise HTTPException(status_code=404, detail="Reading unit not found for section")
    ...
    xp_earned = 0
    completed = list(memory.units_completed or [])
    if status == ReadingUnitStatus.COMPLETED and unit.id not in completed:
        completed.append(unit.id)
        memory.units_completed = completed
        memory.current_unit_id = unit.id
        xp_earned = _award_progress_xp(unit)
        memory.xp_earned = (memory.xp_earned or 0) + xp_earned
    elif status == ReadingUnitStatus.IN_PROGRESS:
        memory.current_unit_id = unit.id
    elif status == ReadingUnitStatus.UNREAD:
        memory.current_unit_id = None if memory.current_unit_id == unit.id else memory.current_unit_id
        memory.units_completed = [value for value in completed if value != unit.id]
