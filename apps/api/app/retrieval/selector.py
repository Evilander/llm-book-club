...
from ..services.reader_text import assemble_reading_text
...
def select_session_slice(
    ...
    # Retrieval still sees the whole selected slice; the introductory context
    # must not grow to the length of a book as a companion visits more chapters.
    context_parts = []
    remaining = max(0, settings.max_context_tokens) * 4
    for section in sections:
        section_chunks = [c for c in chunks if c.section_id == section.id]
        part = f"## {section.title or 'Reading section'}\n" + assemble_reading_text(section_chunks).text
        if settings.max_context_tokens > 0:
            part = part[:remaining]
            remaining -= len(part) + 1
        context_parts.append(part)
        if settings.max_context_tokens > 0 and remaining <= 0:
            break
