...
def _merge_book_metadata(existing: dict | None, extracted: dict | None) -> dict:
...
def _stage_metadata(existing: dict | None, stage: str) -> dict:
    ...
    metadata["stage_started_at"] = datetime.utcnow().isoformat()
    return metadata


def _write_book_stage(
    db: Session,
    book: Book,
    stage: str,
    *,
    ...
    book.metadata_json = _stage_metadata(book.metadata_json, stage)
...
async def run_ingestion_pipeline(
    ...
    try:
        ...
        _write_book_stage(db, book, "extracting", status=IngestStatus.PROCESSING)
        ...
        book.metadata_json = _merge_book_metadata(book.metadata_json, extracted.metadata)
...
def run_ingestion_sync_from_path(book_id: str, file_path: str, filename: str) -> str:
    """Synchronous wrapper that reads a stored/local file inside the worker."""
    import asyncio

    data = Path(file_path).read_bytes()
    return asyncio.run(run_ingestion_pipeline(book_id, data, filename))
